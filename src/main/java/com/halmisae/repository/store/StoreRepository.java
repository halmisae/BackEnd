...
public interface StoreRepository extends JpaRepository<Store, Integer> {
    ...
    @Query("SELECT new com.halmisae.dto.user.StoreReadDetailDTO(s.storeNumber, s.storeName, s.image, s.closingDiscount.closingPrice, s.weekdayOpen, s.weekdayClose, s.breakStart, s.breakEnd, s.closingDiscount.pickupTime, s.address, s.addressDetail, s.closingFood.quantity) " +
            "FROM Store s " +
            "WHERE s.storeNumber = :storeNumber")
    StoreReadDetailDTO findStoreWeekdayDetail(@Param("storeNumber") int storeNumber);

    @Query("SELECT new com.halmisae.dto.user.StoreReadDetailDTO(s.storeNumber, s.storeName, s.image, s.closingDiscount.closingPrice, s.weekendOpen, s.weekendClose, s.breakStart, s.breakEnd, s.closingDiscount.pickupTime, s.address, s.addressDetail, s.closingFood.quantity) " +
            "FROM Store s " +
            "WHERE s.storeNumber = :storeNumber")
